#include <ESP8266WiFi.h>
#include <ESP8266HTTPClient.h>
#include <WiFiClient.h>
#include <ArduinoJson.h>

// WiFi credentials
const char* ssid = "WIFI_SSID_REDACTED";
const char* password = "WIFI_PASS_REDACTED";

// API endpoints
const char* login_endpoint = "http://www.agripeweb.com:8080/v1/Auth/LogIn";
const char* api_endpoint = "http://www.agripeweb.com:8080/v1/reads/add";

// MPX10DP sensor pin
#define SENSOR_PIN A0
#define SENSOR_PIN_ACTIVE 2
#define NUM_AMOSTRAS 5
#define INTERVALO_LEITURAS 1000
#define SLEEP_TIME 60e6 // 60 seconds

String token = "";  // Variável global para guardar o token

void connectToWiFi() {
  Serial.println("Connecting to WiFi...");
  WiFi.begin(ssid, password);
  int attempts = 0;
  while (WiFi.status() != WL_CONNECTED && attempts < 20) {
    delay(500);
    Serial.print(".");
    attempts++;
  }
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConnected to WiFi");
    Serial.print("IP address: ");
    Serial.println(WiFi.localIP());
    Serial.print("MAC address: ");
    Serial.println(WiFi.macAddress());    
  } else {
    Serial.println("\nFailed to connect to WiFi");
  }
}

float readMPX10DP() {
  float total = 0;
  for (int i = 0; i < NUM_AMOSTRAS; i++) {
    total += analogRead(SENSOR_PIN);
    delay(INTERVALO_LEITURAS);
  }
  int media = total / NUM_AMOSTRAS;
  return media;
}

// Função para fazer o login e pegar o token
bool getAuthToken() {
  if (WiFi.status() != WL_CONNECTED) return false;

  WiFiClient client;
  HTTPClient http;
  http.begin(client, login_endpoint);
  http.addHeader("Content-Type", "application/json");

  // Montar o corpo JSON para login
  StaticJsonDocument<200> doc;
  doc["email"] = "IOT_EMAIL_REDACTED";       
  doc["password"] = "IOT_PASS_REDACTED";     
  String requestBody;
  serializeJson(doc, requestBody);

  int httpCode = http.POST(requestBody);

  if (httpCode > 0) {
    Serial.printf("Login response code: %d\n", httpCode);
    if (httpCode == HTTP_CODE_OK) {
      String response = http.getString();
      Serial.println("Login response: " + response);

      StaticJsonDocument<512> responseDoc;
      DeserializationError error = deserializeJson(responseDoc, response);
      if (!error) {
        if (responseDoc.containsKey("token")) {
          token = responseDoc["token"].as<String>();
          Serial.println("Token obtained: " + token);
          http.end();
          return true;
        }
      }
    }
  } else {
    Serial.printf("Login failed, error: %s\n", http.errorToString(httpCode).c_str());
  }

  http.end();
  return false;
}

void sendSensorData() {
  if (WiFi.status() == WL_CONNECTED && token.length() > 0) {
    WiFiClient client;
    HTTPClient http;

    StaticJsonDocument<200> doc;
    doc["code"] = WiFi.macAddress();
    doc["value"] = readMPX10DP();
    String requestBody;
    serializeJson(doc, requestBody);

    http.begin(client, api_endpoint);
    http.addHeader("Content-Type", "application/json");
    http.addHeader("Authorization", "Bearer " + token);  // Token no cabeçalho

    int httpCode = http.POST(requestBody);

    if (httpCode > 0) {
      Serial.printf("POST response code: %d\n", httpCode);
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_CREATED) {
        String payload = http.getString();
        Serial.println("Response: " + payload);
      }
    } else {
      Serial.printf("POST failed, error: %s\n", http.errorToString(httpCode).c_str());
    }

    http.end();
  } else {
    Serial.println("WiFi not connected or token missing, skipping API call");
  }
}

void activateSensor() {
  digitalWrite(SENSOR_PIN_ACTIVE, HIGH);
}

void desactivateSensor() {
  digitalWrite(SENSOR_PIN_ACTIVE, LOW);
}

void setup() {
  pinMode(SENSOR_PIN_ACTIVE, OUTPUT);
  activateSensor();
  delay(1000);

  Serial.begin(115200);
  delay(100);

  connectToWiFi();

  if (getAuthToken()) {
    sendSensorData();
  } else {
    Serial.println("Token not obtained, skipping data send.");
  }

  WiFi.disconnect(true);
  delay(1);

  desactivateSensor();

  Serial.println("Entering deep sleep for 60 seconds...");
  ESP.deepSleep(SLEEP_TIME, WAKE_RF_DISABLED);
}

void loop() {
  // Empty
}