...
{
    public class NdviOverlayImageServiceTests
    {
        ...
        {
            public required NdviOverlayImageService Svc { get; init; }
        ...
        private static Deps Build(
            PlatformAiSettings? settings, string? token, byte[]? png = null, byte[]? cached = null)
        {
            ...
            process.Setup(p => p.GetNdviOverlayPngAsync(It.IsAny<string>(),
            ...
            return new Deps
            {
                Svc = new NdviOverlayImageService(db.Object, tokenProvider.Object, process.Object, store.Object, NullLogger<NdviOverlayImageService>.Instance),
        ...
        {
            var reading = new NdviReading { Id = 1, AreaId = 5, OverlayImageFileId = ObjectId.GenerateNewId() };
            var d = Build(Enabled(), token: "t", cached: [1, 2, 3]);

            var result = await d.Svc.GetOrCreateOverlayAsync(Area(), reading, CancellationToken.None);
        ...
        [Fact]
        public async Task GetOrCreate_CeuLimpoSemPng_Gera_Grava_SetaFileId()
        {
            var reading = new NdviReading { Id = 1, AreaId = 5, AcquisitionDate = new DateTime(2026, 7, 18, 23, 24, 0, DateTimeKind.Utc) };
            var d = Build(Enabled(), token: "t", png: [9, 9, 9, 9]);

            var result = await d.Svc.GetOrCreateOverlayAsync(Area(), reading, CancellationToken.None);

            Assert.Equal([9, 9, 9, 9], result);
            d.Store.Verify(o => o.UploadAsync(It.IsAny<byte[]>(), It.IsAny<string>(), "image/png", It.IsAny<CancellationToken>()), Times.Once);
        ...
        [Fact]
        public async Task GetOrCreate_Nublada_Null_NaoGera()
        {
            // Passagem nublada não tem pixel válido → sem imagem, e sem gastar PU tentando.
            var reading = new NdviReading { Id = 1, AreaId = 5, AcquisitionDate = DateTime.UtcNow, CloudRejected = true };
            var d = Build(Enabled(), token: "t", png: [1]);

            Assert.Null(await d.Svc.GetOrCreateOverlayAsync(Area(), reading, CancellationToken.None));
            d.Process.Verify(p => p.GetNdviOverlayPngAsync(It.IsAny<string>(),
                It.IsAny<GeoJsonPolygon<GeoJson2DGeographicCoordinates>>(),
                It.IsAny<DateTime>(), It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Never);
        }
        ...
        public async Task GetOrCreate_JanelaDoBucket_SemTruncarAHora()
        {
            ...
            await d.Svc.GetOrCreateOverlayAsync(Area(), reading, CancellationToken.None);
        ...
        [Fact]
        public async Task GetOrCreate_ProcessDevolveVazio_Null()
        {
            var reading = new NdviReading { Id = 1, AreaId = 5, AcquisitionDate = DateTime.UtcNow };
            var d = Build(Enabled(), token: "t", png: []); // PNG de 0 bytes (transparente/erro silencioso)
