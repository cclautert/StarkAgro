# IoT Lead StarkAgro — Heartbeat Checklist
...
2. Abrir workspace na raiz **StarkAgro**; paths de firmware em `StarkAgroIOT/`.
...
| Diff recente em `StarkAgroIOT/` no repo | Review mental: credenciais vazaram? contrato API mudou? |
...
- [ ] **Secrets scan:** grep em `StarkAgroIOT/` por padrões `password`, `ssid`, `token = "` com valores reais → `[Alert]` CEO se encontrado no branch principal.
- [ ] **Contrato API:** comparar firmware (login + add read) com rotas em `StarkAgroAPI` — divergência → issue Backend com título claro.
