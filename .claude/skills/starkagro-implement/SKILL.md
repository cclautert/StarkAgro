---
name: starkagro-implement
description: |
  Full feature implementation workflow for the StarkAgro project using a GitHub Issue number.
  ...
  a GitHub Issue number and wants the full development workflow executed on the StarkAgro project.
  Also trigger when the user says things like "build this feature", "start work on issue #NNN", or "do the
  implementation for NNN" in the context of StarkAgro.
---

# StarkAgro — Implement Feature from GitHub Issue
...
```
get_issue(owner="cclautert", repo="StarkAgro", issue_number=NNN)
list_issue_comments(owner="cclautert", repo="StarkAgro", issue_number=NNN)
```
...
```
Issue: https://github.com/cclautert/StarkAgro/issues/NNN
```
...
Run `dotnet build StarkAgroAPI/StarkAgroAPI.csproj` when complete and fix any compilation errors.
...
1. Create new entity classes in `StarkAgroAPI/Models/Entities/` inheriting from `Entity`
...
Run `dotnet build StarkAgroAPI/StarkAgroAPI.csproj` to verify.
...
- **Minimum 90% line coverage** on all production code added or modified in this issue (measure with `--collect:"XPlat Code Coverage"`; see `starkagro-test-writer` skill).
...
- Test files go in `StarkAgroAPI.Tests/` mirroring the production folder structure
...
Run `dotnet test StarkAgroAPI.Tests/StarkAgroAPI.Tests.csproj` and fix any failures.
...
| Field | Value |
|---|---|
| Issue | #NNN — https://github.com/cclautert/StarkAgro/issues/NNN |
...
Post via `create_issue_comment(owner="cclautert", repo="StarkAgro", issue_number=NNN)`:
