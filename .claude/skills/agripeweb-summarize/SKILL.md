---
name: agripeweb-summarize
description: |
  Generates a summary of the current AgripeWeb branch changes and optionally posts it to a
  ...
  GitHub issue", "generate PR description", or "what changed in this branch" in the AgripeWeb
  project context. Also trigger when the user provides an issue number and wants a summary posted.
---

# AgripeWeb — Branch Change Summary
...
git diff $(git merge-base HEAD main) HEAD -- AgripeWebAPI/Controllers/
git diff $(git merge-base HEAD main) HEAD -- AgripeWebAPI/Domain/Handlers/
git diff $(git merge-base HEAD main) HEAD -- AgripeWebAPI/Models/Entities/
```
...
Only call `create_issue_comment(owner="cclautert", repo="AgripeWeb", issue_number=NNN)` if the user answers yes.
