...
{
    public class GetListReadByPivotIdHandlerTests
    {
        ...
        public GetListReadByPivotIdHandlerTests()
        {
        ...
        {
            // Arrange & Act & Assert
            Assert.Throws<ArgumentNullException>(() => new GetListReadByPivotIdHandler(null!));
        }

        [Fact]
        public async Task Handle_SensorNotFound_ReturnsEmpty()
        {
            // Arrange
            MongoMockHelper.SetupFind<Sensor>(_mockSensors, null);

            var handler = new GetListReadByPivotIdHandler(_mockDbContext.Object);
            var request = new GetAllListReadByPivotIdRequest { SensorId = 99, Quadrante = 1, NumberOfReads = 10 };
            ...
            var items = new List<GetAllReadByPivotIdResponse>();
        ...
        {
            // Arrange
            var sensor = new Sensor { Id = 1, PivoId = 10, Quadrante = 2, Name = "Sensor1", Code = "S1" };
            MongoMockHelper.SetupFind(_mockSensors, sensor);

            var expectedReads = new List<GetAllReadByPivotIdResponse>
            {
                new() { Id = 100, SensorId = 1, Value = 25.5m, Date = DateTime.UtcNow.AddHours(-1) },
                new() { Id = 101, SensorId = 1, Value = 30.0m, Date = DateTime.UtcNow }
            };
            MongoMockHelper.SetupFindProjection<ReadSensor, GetAllReadByPivotIdResponse>(_mockReadSensors, expectedReads);

            var handler = new GetListReadByPivotIdHandler(_mockDbContext.Object);
            var request = new GetAllListReadByPivotIdRequest { SensorId = 1, Quadrante = 2, NumberOfReads = 10 };
