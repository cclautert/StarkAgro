...
| Zoneamento de adubação (GeoTIFF) | `GET /v1/areas/{id}/zones/{readingId}` baixa um **GeoTIFF de zonas** (classe por pixel, 8 bits, 1 banda, georreferenciado) — mapa de prescrição p/ maquinário. `CdseProcessService.ZonesEvalscript` (gerado de `NdviClassification.BuildClassIndexFunction`, mesmos cortes do PNG/legenda) com `format: image/tiff` + `sampleType: UINT8`; nuvem/no-data → **255**. **Gerado SÓ no download** por `NdviZoneService` (nunca no fetch) e **cacheado** em `NdviReading.ZoneImageFileId` (paga PU 1×/reading). `GetNdviZonesHandler` copia a **dupla checagem de posse** do overlay + exige `OverlayImageFileId` (nublada não rende raster). Kill-switch `Sentinel2Enabled` desligado → null → 404. ⚠️ **Validado contra a CDSE real**: os 4 tags de geo (`GeoKeyDirectory`/`ModelTiepoint`/`ModelPixelScale`/`GeoAsciiParams`) vêm no TIFF |
