...
| Quem paga a conta | Um produtor tem **exatamente um pagante**, por precedência: vínculo `Client` **Active** com revenda → a revenda paga (pool); senão o próprio produtor (`User.DiagnosisPlanId`). O **agrônomo nunca é um terceiro caminho de cobrança** — agrônomo que banca a carteira é cadastrado como **revenda de um** (ele vira `Manager` dela) e cai no pool existente; `/agronomo/faturamento` é painel informativo. Cota segue a mesma lógica de "quem paga, limita": `DiagnosisQuotaService` resolve `user.DiagnosisQuotaPerMonth` → `Revenda.DiagnosisQuotaPerMonth` (pelo vínculo `Client` Active, **não** pelo cache `User.RevendaId`) → `PlatformAiSettings.DefaultDiagnosisQuotaPerMonth`. Cuidado: `0 = ilimitado` — deixar o default da plataforma em 0 dá laudo grátis sem teto a quem não tem plano nem revenda |
| Assentos da revenda | `DiagnosisPlan` tem, além do eixo de laudo, o eixo **comercial**: `IncludedMembers`, `MemberOveragePriceCents` e `MaxMembers` (`0` = ilimitado). Assento = vínculo `Client` **Active** ou **Pending não vencido** (convite reserva a vaga; sem o filtro de data um convite morto seguraria o assento, já que o status só vira `Expired` quando alguém tenta aceitar). Gestor e agrônomo **não** ocupam assento. A regra vive em `RevendaSeatService.OccupiesSeat` (estática e pura — o `MongoMockHelper` ignora filtros, então filtro em query não é testável). Excedente de assento é **cobrado**; o teto **bloqueia** o convite em `InviteRevendaMemberHandler`. O aceite segue permissivo: a vaga já foi reservada no convite. Plano legado desserializa com `0` nos três campos → fatura idêntica à de antes |
...
| Overlay NDVI (PNG) | O PNG colorizado (Process API, `CdseProcessService`) vive num bucket GridFS **separado** `ndvi_overlays` (`agpDBContext.NdviOverlays` / `INdviOverlayStore`) — **nunca** no de laudos `diagnosis_images`. É **acessório**: falha da Process API é engolida (`OverlayImageFileId` fica null, tendência não quebra). O worker gera **só** para a passagem mais nova de cada ciclo, mas ⚠️ **qualquer passagem de céu limpo gera o PNG sob demanda ao ser aberta** (`INdviOverlayImageService.GetOrCreateOverlayAsync`, **espelho de `INdviZoneService`**: on-demand + cache em `OverlayImageFileId`, gate `Sentinel2Enabled`, nublada→null; paga PU 1×/reading). Sem isso, navegar o histórico mostrava só o contorno — a maioria das passagens não tem PNG do worker. Servido privado por `GET /v1/areas/{id}/overlay/{readingId}` via `GetNdviOverlayImageHandler` com **dupla checagem de posse** (área do dono → reading da área) → `GetOrCreateOverlayAsync`. O `NdviTrendResponse` manda **bbox em toda passagem não-nublada** (`[minLng,minLat,maxLng,maxLat]`), não só nas que já têm PNG — o front alinha o `L.imageOverlay` e pede a imagem para qualquer passagem de céu limpo (`readingId`, não `overlayReadingId`). ⚠️ Como o overlay passa a existir sob demanda, o **GeoTIFF de zonas** (que exige `OverlayImageFileId`) fica disponível para qualquer passagem depois de aberta |
| Histórico retroativo NDVI (F7) | `GET /v1/areas/{id}/history?date=yyyy-MM-dd` (`FetchNdviHistoryHandler`) deixa o usuário "voltar no tempo" numa data fora do histórico armazenado. **Modo híbrido**: o front navega as passagens já armazenadas de graça (o `area-detail` agora é dirigido por uma passagem `selected`, não mais só pela mais recente — setinhas ◀/▶ + `<input type=date>`); só chama a API para datas fora do range. A ordem dos guardas no handler é **barato→caro**: posse → data no passado → **curto-circuito grátis** se a janela já tem leitura → **teto mensal** (`NdviMonthlyBudgetCents`, mesmo freio do worker) → só então a chamada paga à CDSE. ⚠️ **NÃO reusa `FetchAsync`** — o dedup dela pula `dateKey <= LastAcquisitionDate`, justo as passagens antigas; `FetchHistoricalAsync` usa **janela explícita centrada** (`NdviFetchService.HistoryWindow`, ~±3 d, `to` nunca no futuro), deduplica **só** pelo índice único `{AreaId, AcquisitionDate}` e **não toca** em `LastAcquisitionDate`/`NextFetchAt` (retroativo não pode perturbar a cadência do worker). Persiste no mesmo `ndvi_readings` com `NdviCostCents` → conta no `NdviCostService`/`/admin/ia`, e re-consultar a mesma data vira **grátis** (curto-circuito + dedup). Overlay é gerado para a passagem **mais próxima da data pedida** (não a mais nova). O mapeamento de campos de `NdviReading` foi extraído para `PersistPassageAsync`, compartilhado com `FetchAsync` |
| Sentinel-1 (radar, F5) | `Sentinel1Enabled` (`platform_ai_settings`, `/admin/ia`) liga o `Sentinel1Processor` (worker), que busca a série de radar (RVI = 4·VH/(VV+VH) + VV/VH crus) na **mesma Sentinel Hub API** do NDVI. ⚠️ **Serviço SEPARADO `CdseSentinel1Service`, NÃO parametrizei o `CdseStatisticalService`** — o S1 tem evalscript, `dataFilter` (sem nuvem) e parsing próprios; enfiar no do NDVI arriscaria o caminho S2. `dataFilter`: `acquisitionMode:IW, polarization:DV, orbitDirection` + `processing.backCoeff:GAMMA0_ELLIPSOID` (sem `maxCloudCoverage` — radar não tem nuvem). ⚠️ **Órbita FIXA `DESCENDING`** (const `Sentinel1FetchService.OrbitDirection`), gravada em cada leitura e **na chave única** `{AreaId, AcquisitionDate, OrbitDirection}` — misturar asc/desc faz o backscatter pular por geometria de visada, não por lavoura. ⚠️ **Worker itera + dedup (padrão FireWatch/Climate), NÃO clona o claim atômico do `NdviProcessor`** — o claim usa os campos de worker do `MonitoredArea`, que são do NDVI; dois processors clamando brigariam. O `Sentinel1FetchService` deriva a última passagem das próprias leituras e **pula a chamada à CDSE se < 5 dias** (não paga PU por nada novo). Coleção `sentinel1_readings`. Custo fator PU 1,0 (3 bandas), gasto do mês em `/admin/ia`. Front: aba "RVI (radar)" no `area-detail`, série própria (datas diferentes do NDVI). ⚠️ **Validado contra a CDSE real** (rvi=0.70, 7 passagens/mês → ciclo ~6 d do S1C+S1D) |
| Zoneamento de adubação (GeoTIFF) | `GET /v1/areas/{id}/zones/{readingId}` baixa um **GeoTIFF de zonas COLORIDO** (RGB, as cores das classes, georreferenciado) — abre legível em qualquer visualizador **e** no QGIS. `CdseProcessService.ZonesEvalscript` reusa o `ramp()` de `NdviClassification` (as mesmas cores do overlay/legenda) com `format: image/tiff`, `output bands:3`; fora do talhão/nuvem → **branco** `[1,1,1]`. ⚠️ **Era índice cinza 0-5 (UINT8, 1 banda) — trocado para RGB colorido** porque abria como um borrão preto-e-branco em visualizador de celular (os valores 0-5 são quase pretos). Se um dia precisar do índice cru p/ prescrição de maquinário, o caminho é uma paleta embutida (não trivial sem GDAL). **Gerado SÓ no download** por `NdviZoneService` (nunca no fetch) e **cacheado** em `NdviReading.ZoneImageFileId` (paga PU 1×/reading). `GetNdviZonesHandler` copia a **dupla checagem de posse** do overlay + exige `OverlayImageFileId` (nublada não rende raster). Kill-switch `Sentinel2Enabled` desligado → null → 404. ⚠️ **Validado contra a CDSE real**: os 4 tags de geo (`GeoKeyDirectory`/`ModelTiepoint`/`ModelPixelScale`/`GeoAsciiParams`) vêm no TIFF |
| Culturas (lista gerida) | Cultura **não é campo livre** — é um seletor alimentado pela coleção `cultures` (entidade `Culture`, **dado global** como `fertilization_profiles`). CRUD admin em `AdminController` (`v1/admin/cultures`, tela `/admin/culturas`); leitura para **qualquer autenticado** em `GET /v1/cultures` (`CulturesController`) — os 3 seletores (área, perfil de adubação, diagnóstico) leem daí, **é o que faz `area.Crop` casar exato com `FertilizationProfile.Culture`** (mata o bug "Café"≠"café"). ⚠️ **Índice único case-insensitive** (`Collation` strength 2) impede "Café"/"café" na lista. **Já nasce com as culturas comuns do BR** (`CultureSeed.Canonical`) + **backfill** dos valores em uso (áreas/perfis/diagnósticos) no boot (`SeedCulturesAsync`, idempotente, **não ressuscita** canônica que o admin apagou). **Delete permitido mesmo em uso** — cada `<select>` inclui o valor salvo mesmo fora da lista (não perde a cultura na edição). Sem validação server-side de `Crop ∈ cultures` (evita paradoxo delete-em-uso); `FertilizationProfileResolver` case-insensitive é o backstop |
| Perfis de adubação (NPK) | Coleção `fertilization_profiles` (cultura → dose NPK kg/ha por classe), **dado global de plataforma** como `diagnosis_plans`: CRUD só no `AdminController` (`v1/admin/fertilization-profiles`), sem filtro por tenant. Tela `/admin/adubacao`. **`ClassKey` casa com `NdviClassification.Classes[].Key`** — a zona do TIFF vira prescrição. ⚠️ **Nasce vazio; nenhuma dose inventada** — agronomia é parâmetro, o admin/agrônomo preenche |
| Prescrição de adubação (relatório) | `GET /v1/areas/{id}/prescription/{readingId}?profileId=` (`GetFertilizationPrescriptionHandler`) **cruza** o perfil NPK da cultura × a distribuição de classes da passagem (`ClassCounts`) × os hectares do talhão → dose e kg por zona + **total do talhão**. **Custo zero de CDSE** (só dado já armazenado). Cultura resolvida **automática** pelo `area.Crop` via `FertilizationProfileResolver` (regra pura **compartilhada** com o GeoTIFF de doses — não podem divergir: case-insensitive+trim, override por `profileId`); sem cultura/sem perfil → notifica (o front linka `/admin/adubacao`). Hectares vêm de `AreaHectares.Of` (helper **puro**, fórmula geodésica de polígono esférico sobre o anel — o círculo já chega como anel). ⚠️ **Área distribuída só entre pixels válidos** (exclui nuvem) → `CloudCoveragePct` viaja para a tela sinalizar imprecisão. Classe **sem dose** no perfil entra com 0 + `HasDose=false` (não some em silêncio). Painel no `area-detail` dirigido pela **passagem selecionada** (Imprimir/CSV client-side) |
| Prescrição de doses (GeoTIFF taxa variável) | `GET /v1/areas/{id}/prescription/{readingId}/geotiff?profileId=&nutrient=` (`GetFertilizationPrescriptionTiffHandler`) baixa um **GeoTIFF FLOAT32** onde o **valor do pixel = dose kg/ha** (3 bandas N/P/K; `nutrient=N\|P\|K` → 1 banda), para taxa variável no trator. A dose é **função-degrau do NDVI** pelos **mesmos cortes** de `NdviClassification.Classes` — `DoseEvalscript.Build` (puro) gera o evalscript a partir do `FertilizationProfile` (fora do talhão/nuvem → 0; classe sem dose → 0). Reusa a Process API do zones (`CdseProcessService.GetPrescriptionTiffAsync`, `BuildBody` com `image/tiff`). ⚠️ **NÃO cacheado** (ao contrário do zones colorido em `ZoneImageFileId`): a dose depende do perfil, que muda — cachear viraria prescrição **errada e silenciosa** no campo. `NdviPrescriptionRasterService` é espelho de `NdviZoneService` **sem o cache**; gate `Sentinel2Enabled`; **paga PU por download**. Posse dupla + nublada→404 (raster todo-zero). ⚠️ **`sampleType: FLOAT32` é peça nova — validar contra a CDSE real** (200 + 4 tags de geo + abre no QGIS com valores, não cor): é onde um 400/raster vazio só apareceria na API real |
| Alertas climáticos (geada / calor) | `ClimateAlertsEnabled` + `FrostAlertTempC` (3) + `HeatAlertTempC` (35) (`platform_ai_settings`, `/admin/ia`) ligam o `ClimateWatchProcessor` (worker próprio, kill-switch independente do fogo). **Custo zero** — Open-Meteo gratuito. ⚠️ **Usa `IAgricultureWeatherService.GetAgricultureDataAsync` (tem temperatura), NÃO `IWeatherForecastService.GetForecastAsync`** (só precipitação — a armadilha do issue). Centroide via `AreaCentroid.Of` (puro): `CenterLat/Lng` no círculo, centro do bbox no polígono, null sem geometria (área pulada). Coleção `climate_alerts` com **índice único** `{AreaId, AlertType, ForecastDate}` — previsão reavaliada a cada tick não redispara. ⚠️ **`HeatAlertTempC == 0` de doc legado cairia em "máx ≥ 0" = dispara sempre**; o worker faz fallback para 35 (frost em 0 é conservador, fica como veio). Sino: `FrostRisk`/`HeatRisk` (`GetUserAlertsHandler`, id `climate-{areaId}-{id}` para o clique abrir a área); um push por área por tick |
| Fire Shield (focos de calor, NASA FIRMS) | `FireAlertsEnabled` + `FirmsMapKey` (`platform_ai_settings`, tela `/admin/ia`) ligam o `FireWatchProcessor` (worker), que busca focos VIIRS 375 m no bbox de cada área `MonitoringEnabled`, expandido por `FireAlertRadiusKm` (default 10). **Custo zero** — FIRMS é gratuito; a flag é freio de ruído/rate-limit (5.000 req/10 min), não de dinheiro. Coleção `fire_hotspots` com **índice único** `{AreaId, Latitude, Longitude, AcquiredAt, Satellite}` — reentrega do NRT é no-op (mesmo padrão do NDVI). ⚠️ **Resposta é CSV, não JSON**, e o parser (`FirmsHotspotService.ParseCsv`) mapeia **por nome de coluna**, nunca por posição — reordenar/adicionar coluna não desalinha, coluna obrigatória faltando → vazio. O FIRMS devolve **200 com texto de erro** quando a MAP_KEY é inválida (sem cabeçalho CSV) → tratar como falha, não lista vazia. ⚠️ **Latência honesta ~1 h** no Brasil (VIIRS RT/NRT; o <5 min do deck é só EUA/Canadá) — a UI declara isso. Alerta no sino agrupado por `{área, dia}` (`GetUserAlertsHandler`, `AlertType = "FireHotspot"`, id `fire-{areaId}-{yyyyMMdd}`); **um push por área por tick**, nunca um por foco. `FireAreaBbox` (puro) expande o bbox e mede distância |
| NDRE + NDMI (índices extras, F1) | `ExtraIndicesEnabled` (`platform_ai_settings`, tela `/admin/ia`) liga NDRE (B05, red-edge — não satura em dossel denso) e NDMI (B11, SWIR — umidade do dossel) **na mesma requisição** do NDVI. Desligado, o evalscript volta a pedir só as 4 bandas do NDVI. Duas coisas a saber: (1) **6 bandas de entrada → fator PU 2,0** contra 1,33; o custo é **proxy manual** (`NdviCostCents`), **não escala sozinho** — subir o valor ao ligar a flag. (2) A coleção `ndvi_readings` e a entidade `NdviReading` passam a guardar **três** índices (`Ndre*`/`Ndmi*` além de `Ndvi*`); **dívida de nome assumida** — renomear custaria migração por ganho cosmético. Sem migração: leitura buscada com a flag off desserializa NDRE/NDMI como **0**, e o consumidor distingue "sem dado" por data/nuvem, **nunca pelo valor** (NDRE de vegetação real pode ser ~0). Classificação por classes segue **só no NDVI**; NDRE/NDMI têm só a linha de tendência (seletor no `area-detail`, escondido quando nenhuma leitura tem o índice). ⚠️ **NDMI, não NDWI** — o NDWI de McFeeters `(B03−B08)/…` é para água; umidade de dossel é NDMI/NDWI-Gao com SWIR |
| Níveis de NDVI (classes de biomassa) | `Services/Ndvi/NdviClassification.cs` é a **única fonte da verdade** de cortes e cores (6 classes, Solo Exposto <0,20 → Alta ≥0,80). Ela alimenta **três** coisas ao mesmo tempo: os `bins` do histograma pedido à Statistical API, o `ramp()` do evalscript que colore o PNG (`CdseProcessService.Evalscript` é **gerado**, não escrito à mão) e o `Classes` do `NdviTrendResponse` que vira a legenda no front. Editar o ramp/legenda direto, sem passar por ela, **faz o mapa mentir para a legenda** e nenhum teste de rota pega. Classe estática e pura, como `MonitoredAreaGeometry` |
| Histograma NDVI é de graça | A contagem de pixels por classe vem de `calculations.ndvi.histograms.default.bins` na **mesma** requisição Statistical que já buscava a média — **zero Processing Unit extra, zero chamada nova**. Gravado em `NdviReading.ClassCounts` (chave estável, **não** índice — reordenar as classes não pode reinterpretar leitura antiga). **Não há migração**: reading gravada antes disso fica com a lista vazia e a tela cai no fallback; refazer custaria PU. Por isso a legenda do mapa só aparece quando a passagem exibida **tem** `classes` — PNG antigo foi colorido com o ramp velho, e anunciar cortes que a imagem não usou seria pior que não ter legenda |
...
| Deploy | `main` + green CI; secrets only via env / user secrets — placeholders in repo. **Auto-deploy é opt-in**: o job pós-CI (`deploy.yml`) só roda se a var de repo `AGRIPEWEB_AUTO_DEPLOY == 'true'` **e** os secrets da VPS (`VPS_HOST`/`VPS_USER`/`VPS_SSH_KEY`/`VPS_DEPLOY_PATH`) estiverem setados — senão o merge não dispara deploy (evita falha "missing server host"). Deploy manual: `gh workflow run Deploy` ou `scripts/redeploy-starkagro.sh` |
| `.env` da VPS e o Compose | O `.env` real vive em `/opt/starkagro/.env`, mas o Compose procura o `.env` no **diretório do arquivo compose** (`/opt/starkagro/docker/`). Rodar `docker compose -f docker/docker-compose.vps.yml up -d` **sem `--env-file .env`** faz toda `${VAR}` virar string vazia: o stack sobe sem Mongo/JWT/MQTT, o `/health` responde 200 e só o log denuncia (`IDX10703: key length is zero`). Proteções: symlink `docker/.env -> ../.env` (recriado a cada deploy) e a checagem pós-deploy no `redeploy-starkagro.sh`, que aborta se algum segredo chegar vazio no container |
...
- CORS: `localhost:4200` (dev), `starkagro.com.br` (prod)  
