...
| Prescrição de adubação (relatório) | `GET /v1/areas/{id}/prescription/{readingId}?profileId=` (`GetFertilizationPrescriptionHandler`) **cruza** o perfil NPK da cultura × a distribuição de classes da passagem (`ClassCounts`) × os hectares do talhão → dose e kg por zona + **total do talhão**. **Custo zero de CDSE** (só dado já armazenado). Cultura resolvida **automática** pelo `area.Crop` (case-insensitive+trim), com override opcional por `profileId`; sem cultura/sem perfil → notifica (o front linka `/admin/adubacao`). Hectares vêm de `AreaHectares.Of` (helper **puro**, fórmula geodésica de polígono esférico sobre o anel — o círculo já chega como anel). ⚠️ **Área distribuída só entre pixels válidos** (exclui nuvem) → `CloudCoveragePct` viaja para a tela sinalizar imprecisão. Classe **sem dose** no perfil entra com 0 + `HasDose=false` (não some em silêncio). Painel no `area-detail` dirigido pela **passagem selecionada** (Imprimir/CSV client-side). ⚠️ **Só relatório** — o GeoTIFF de doses para taxa variável é fase futura (raster de valores, não-trivial sem GDAL) |
